...
fetch(PRODUCT_INFO_URL + prodID + EXT_TYPE)
  .then((resp) => resp.json())
  .then((data) => {
    var añadirAlCarrito = document.getElementById('añadirCarrito')
     añadirAlCarrito.onclick = function(){
      console.log('click');
     }
    ...
    function showCarrousel() {
      let carrousel = ` <div id="carouselExampleControls" class="carousel slide " data-bs-ride="carousel">
...
<div class="carousel-inner ">
    <div class="carousel-item active  ">
      <img src="${data.images[0]}" class="d-block w-50 m-auto rounded-circle" alt="...">
    </div>`;
      for (i = 1; i < data.images.length; i++) {
        carrousel += `
    
     <div class="carousel-item">
      <img src="${data.images[i]}" class="d-block w-50 m-auto rounded-circle" alt="...">
    </div>`;
      }
      carrousel += `
  </div>
...
<img onclick="setProdID(${relatedProds.id})"class="card-img-top col-4" style="width: 19rem;" src="${relatedProds.image}" alt="Card image cap">
...
<h4>Comentarios</h4>`;
    ...
    for (let comment of data) {
      document.getElementById("container2").innerHTML += `
        ...
        <div class="row">
        <div class="col-12">
        <strong>${comment.user}</strong> - ${comment.dateTime} - ${showStars(
        comment.score
      )}
    ...
    commentBtn.addEventListener("click", function crearComentario() {
      ...
      if (
        ...
        document.getElementById("container2").innerHTML += `
            ...
            }</strong> - ${fecha} ${hora} - ${showStars(prodScore.value)}
            <p>${textArea.value}</p>
            </div>
            </div>
            </div>`;
...
function showAlerts() {
  if (
    ...
    alertify.alert(
      '<i class="fa fa-close" style="font-size:30px;color:red;"></i><strong style="font-size:23px;"> Error al comentar</strong>',
      '<p style="font-size: 18px;">Debes iniciar sesión para poder comentar</p>',
      "Iniciar Sesion"
    );
  } else if (
    (localStorage.getItem("user") != null ||
    ...
    document.getElementById("product-info-textarea").style.border =
      "solid 2px red";
...
document
  .getElementById("product-info-textarea")
...
function addCartProduct(data) {
  ...
  console.log(data)
  var productos = JSON.parse(localStorage.getItem("cartProduct"));
...
function showUser() {
  document.getElementById(
    "showUser"
  ).innerHTML = ` <a class="nav-link dropdown-toggle text-dark fw-bold" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
    ${localStorage.getItem("user") || sessionStorage.getItem("user")}
    </a>
...
function sesionOn() {
  if (localStorage.getItem("user") || sessionStorage.getItem("user"))
    ...
    document.getElementById(
      "showUser"
    ).innerHTML = `<button type="button" class="button-15" id="indexBtn" role="button">Iniciar Sesion</button>`;
